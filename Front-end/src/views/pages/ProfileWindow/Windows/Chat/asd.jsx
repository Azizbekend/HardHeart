...
export default function Chat() {
    ...
    const [idUserNow, setIdUserNow] = useState(idUserFriend.split("_").length > 1 ? idUserFriend.split("_")[1] : idUserFriend.split("_")[0]);
    const [idClaimNow, setIdClaimNow] = useState(idUserFriend.split("_").length > 1 && idUserFriend.split("_")[0]);
    ...
    function postTime() {
        ...
        // console.log({ id: Number(idClaimNow), time: formatDate(currentDate) })

        postFetch('postTime', { id: Number(idClaimNow), timer: currentDate })
            .then((data) => setTimerStart(true));
    }

    const handleBlockToggle = () => {
        const apiName = isBlocked ? "razBlockUser" : "blockUser";
        postFetch(apiName, { idUser: Number(idUserNow) })
    ...
    useEffect(() => {
        if (idChat != ":idChat") {
            fetch(`http://127.0.0.1:8000/api/getMessages?idChat=${idChat}&idUserFriends=${Number(idUserNow)}`, {
                method: 'GET',
                headers: {
                    ...
                    (<>
                        <div className="chat__window">
                            <div className="chat__header">
                                ...
                                {isWideLinkBack ? (
                                    user.role === "user" ? (
                                        <Link to="/profileWindow/adminChats/">← <span>назад</span></Link>
                                    ) : (
